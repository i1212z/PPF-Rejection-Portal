...
export default function TallyRejectedPage() {
  ...
  const tickets = useMemo(() => allTickets.filter((t) => t.status === 'rejected'), [allTickets]);
  ...
  return (
    <div className="space-y-6 min-w-0 max-w-full">
      <div>
        <h2 className="text-lg font-semibold text-gray-900">Rejected tickets (Tally)</h2>
        <p className="text-sm text-gray-500">
          All rejected records. Tick &quot;Update to Tally&quot; for records to be synced to Tally.
        </p>
      </div>
      <Card title="Rejected records" subtitle="Grouped by delivery. Tick to mark for Tally update.">
        {loading ? (
          <p className="text-sm text-gray-500 py-4">Loading…</p>
        ) : groups.length === 0 ? (
          <p className="text-sm text-gray-500 py-4">No rejected tickets.</p>
        ) : (
